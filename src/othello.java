...
public class othello {
	...
	public static String columnString(int col) {
		...
		case 8:
			...
			break;
		default:
			col_string = Integer.toString(col);
			break;
		}
		return col_string;
	}
	...
	public static void main(String[] args) {
		...
		while (!command.equals("exit")) {
			...
			if (MoveSet.size() == 0) {
				...
				print_board(board);
				System.out.printf("Score: White:%d Black:%d", agent.count_color(board, true),
						agent.count_color(board, false));
				break;
			}
			if (turn) {
			...
			System.out.printf("Suggested Move: %d%s\n", suggested.getRow() + 1,
					columnString(suggested.getCollumn() + 1));
			print_board(board);
			/*
			 * Interface for a 2 player game
			 * 
			 	System.out.print("Your move: ");
			   command = input.nextLine(); int column_command =
			   columnInt(Character.toString(command.toCharArray()[1])); int
			   row_command =
			   Integer.parseInt(Character.toString(command.toCharArray()[0]));
			   board = make_move(row_command-1,column_command-1,board,MoveSet);
			 */
			board = make_move(suggested.getRow(), suggested.getCollumn(), board, MoveSet); // comment this out to play the two player game
